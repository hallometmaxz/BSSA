...
* 4.0 - Initialize xmas Teaser Plugin
...
$(function() {
	...
	$("#title").typed({
		strings: ["Merry X-Mas!", "Glüwine", "Yasss", "Snow is life"],
		typeSpeed: 300,
		backDelay: 900,
		loop: true,
		cursorChar: "|",
	...
	4.0 Initialize xmas Teaser Plugin
	================================================================== */
	var $xmasContainer = $(".xmas-container");
	var freezeProp = $xmasContainer.attr("data-freeze-prop") ? parseBool($xmasContainer.attr("data-freeze-prop")) : true;

	// Merge custom animations with the existing animations
	if (typeof xmasCustom !== "undefined") {
		xmasAnimations = $.extend(xmasAnimations, xmasCustom);
	}
	...
	/* Initiate xmas Teaser Plugin */
	$xmasContainer.xmasPlugin({
	...
	$('.pl-skip').on('click' , function(ev) {
		// $xmasContainer.xmasPlugin.pauseTeaser();
		// revealTL.play();
	...
	$('.pl-play').on('click' , function(ev) {
		$xmasContainer.xmasPlugin.togglePlay();
	});
	...
	$(document).on('keydown', function(e) {
		if ($xmasContainer.is(':visible') && !$('#subscribe-page').is(':visible') && e.keyCode === 32) {
			$xmasContainer.xmasPlugin.togglePlay();
		}
	});
	...
	$('.pl-sound').on('click' , function(ev) {
		if($(this).parent().hasClass("highlight")) {
			...
			$xmasContainer.xmasPlugin.toggleSound();
		}
	});
	...
	$xmasContainer.on("STPlay", function () {
		$playIcon.html("PAUSE");
	});

	$xmasContainer.on("STPause", function () {
		$playIcon.html("PLAY");
	});

	$xmasContainer.on("STEnd", function () {
		$playIcon.html("RESTART");
	});

	$xmasContainer.on("STMuted", function () {
		$volumeIcon.addClass("strike");
	});

	$xmasContainer.on("STUnMuted", function () {
